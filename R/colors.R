...
#'
#' @export
color_numeric <- function(palette, domain, na.color = "#808080", alpha = FALSE, reverse = FALSE) {
  rng <- NULL
  if (length(domain) > 0) {
...
color_bin <- function(palette, domain, bins = 7, pretty = TRUE,
  ...
  colorFunc <- color_factor(palette, domain = if (!autobin) 1:numColors,
...
color_quantile <- function(palette, domain, n = 4,
  ...
  # or 5? 4, right?
  colorFunc <- color_factor(palette, domain = 1:(length(probs) - 1),
                           na.color = na.color, alpha = alpha, reverse = reverse)
